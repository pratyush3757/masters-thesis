...
\sethlcolor{pink}\hl{define control flow here\ldots{}}\sethlcolor{correctioncolor}
...
{[}\citeproc{ref-godefroid2012sage}{11}, \citeproc{ref-kern1999formal}{13}, \citeproc{ref-luk2005dynamic}{16}--\citeproc{ref-quinlan2009source}{19}{]}
...
and for that, the techniques come under the term Control Flow Integrity (CFI).{[}\citeproc{ref-sok}{2}{]}
...
As stated by Bruce Schneier: Security is a process, not a product.{[}\citeproc{ref-bruceessay}{20}{]}
...
\sethlcolor{pink}\hl{Mention PAC + BTI discussed in Arm Specifics Section}\sethlcolor{correctioncolor}
...
In AArch64, the available registers are:{[}\citeproc{ref-armasm-user-guide}{4}, \citeproc{ref-arm-isa}{5}{]}
...
The registers and their usage are as follows:{[}\citeproc{ref-armasm-user-guide}{4}, \citeproc{ref-arm-isa}{5}, \citeproc{ref-arm-pcs}{7}{]}
...
We have \sethlcolor{pink}\hl{discussed about stack smashing}\sethlcolor{correctioncolor}, and how arbitrary code execution can be prevented.
...
Automated Exploit Generation (AEG) tools or information disclosure.{[}\citeproc{ref-aeg2014}{8}, \citeproc{ref-riscyrop}{10}{]}
...
Where (lower) \texttt{x} is either \texttt{A} or \texttt{B} depending on the key to be used. More in next section on keys.
There are many more instructions, but we'll just talk about these NOP-space ones.
...
\item
  \texttt{A/B} at the 5th instruction letter determines the key to be used.
\item
...
control flow path through the complete program.{[}\citeproc{ref-laruswpp}{14}, \citeproc{ref-blast}{22}{]}
...
\sethlcolor{pink}\hl{Also write about the constant monitor for input idea}\sethlcolor{correctioncolor}
...
\item
  OS is not trusted if and only if \sethlcolor{pink}\hl{page management?}\sethlcolor{correctioncolor} is handled
