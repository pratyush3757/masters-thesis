...
clean_median = function(x) median(remove_outliers(x))
clean_sd = function(x) sd(remove_outliers(x))
...
df <- data.frame(baseline_median=apply(get_times(baseline_raw), 2, clean_median))
#df$baseline_sd <- apply(get_times(baseline_raw), 2, clean_sd)

df$baseline_pacbti_median <- apply(get_times(baseline_pacbti_raw), 2, clean_median)
#df$baseline_pacbti_sd <- apply(get_times(baseline_pacbti_raw), 2, clean_sd)

df$blast_median <- apply(get_times(blast_raw), 2, clean_median)
#df$blast_sd <- apply(get_times(blast_raw), 2, clean_sd)

df$pacbti_median <- apply(get_times(pacbti_raw), 2, clean_median)
#df$pacbti_sd <- apply(get_times(pacbti_raw), 2, clean_sd)

df$blast_optimised_median <- apply(get_times(blast_optimised_raw), 2, clean_median)
#df$blast_optimised_sd <- apply(get_times(blast_optimised_raw), 2, clean_sd)

df$baseline_pacbti_overhead <- fmt_overhead(df$baseline_median, df$baseline_pacbti_median)
df$blast_overhead <- fmt_overhead(df$baseline_median, df$blast_median)
df$pacbti_overhead <- fmt_overhead(df$baseline_median, df$pacbti_median)
df$blast_optimised_overhead <- fmt_overhead(df$baseline_median, df$blast_optimised_median)
df$pacbti_vs_blast <- fmt_overhead(df$blast_median, df$pacbti_median)
df$blast_optimised_vs_blast <- fmt_overhead(df$blast_median, df$blast_optimised_median)
...
foot_str = "Samples n=1000 for each program, along with CPU_MHZ=1000."
