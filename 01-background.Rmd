...
#### History and Techniques used{#cfahistory}
[Edit/Add to this part]{highlight="pink"}
...
### ARM Assembly Basics
We will only focus on AArch64, the 64-bit instruction set, and on the ARMv8-A profile[@arm-arch-manual].
...
Where (lower) `x` is either `A` or `B` depending on the key to be used. More in next section on keys.
...
- `A/B` at the 5th instruction letter determines the key to be used.
...
### BLAST
As mentioned in [CFA history](#cfahistory), attestation has rarely targeted whole programs.
...
that rely on inline assembly. But as shown by CFA+[@cfaplus], 
