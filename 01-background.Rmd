...
to minimize the software/hardware bugs [@artoftesting; @millerfuzzing;
@luk2005dynamic; @godefroid2012sage; @quinlan2009source; @kern1999formal].
...
Control Flow Integrity (CFI) [@sok].
...
There are 3 main types of attacks that can change the control flow of a program [@sok]:
...
Identification as their ARM implementations later on in section [1.2 - ARM Specifics](#arm-specifics).
...
CFI was originally proposed by Abadi et al.[@abadi2009control] as a policy-based mitigation
against control flow hijacking attacks by restricting the execution of a program
to a pre-computed Control Flow Graph (CFG) [@sok]. This CFG could be statically or
...
Path attestation also acts as a lighter alternative to verifiable computation [@walfish2015verifying].

There are other integrity measurements too that can attest to various properties 
of the system being tested.
Like, weather the system satisfies certain properties [@chen2006protocol; 
@chen2008property; @kil2009remote; @sadeghi2004property],
it booted up with particular software stack [@arbaugh1997secure; @gasser1989digital],
or that it executed certain operations correctly [@sun2020oat].
...
(TEE) like Intel SGX [@intel-sgx] or ARM TrustZone [@armtz] that act as the RoT.
...
in various domains [@abera2019diat; @asokan2015seda; @fraser2004copilot; @li2011viper; @sun2020oat].
As TEEs were generally historically unavailable on low-end embedded devices, 
software based attestation techniques like SWATT [@seshadri2004swatt] and
PIONEER [@seshadri2005pioneer] were proposed.
...
VRASED [@nunes2019vrased] adds integrity checks in software, while using trusted hardware to control the checks.
...
avoid this limitation [@de2021toctou].

Works in path attestation, where the control flow path of the program is recorded 
for the proof, include CFLAT [@abera2016cflat] and OAT [@sun2020oat].
We will focus on these two, as they can represent the main approaches to path 
attestation, and other works have essentially similar methods [@abera2016cflat;
@abera2019diat; @dessouky2018litehax; @huo2020lape; @nunes2021tiny; @sun2020oat;
@toffalini2019scarr; @zhang2021recfa].
...
Table \@ref(tab:logevent-totals) describes the number of TEE domain switches at runtime,
...
programs or to programs that work in specialized environments [@abera2016cflat;
@dessouky2018litehax; @nunes2021tiny; @toffalini2019scarr; @blast].
Prior work has also shown that a Vrf can miss attacks that are directed against
a program when paths in only some parts of it are attested [@hu2019probability].
...
in detail later on in Primary Works - [1.3.1 BLAST](#blast) subsection.
...
runtime auditing [@caulfield2023acfa; @caulfield2024traces] aims to bridge that
...
which is a popular use-case where CFI and CFA techniques are often applied [@sok].
...
and later profiles [@arm-arch-manual].
...
knitr::include_graphics("./imgs/armtz.pdf")
```
...
```{r smc-convention, fig.cap="smc instruction call convention", out.width='100%', fig.align='center', fig.pos='H'}
knitr::include_graphics("./imgs/smc-workflow.pdf")
```
...
```{r optee-arch, fig.cap="OP-TEE Architecture", out.width='80%', fig.align='center', fig.pos='H'}
knitr::include_graphics("./imgs/optee.pdf")
```

## Primary Works

### BLAST
As mentioned in [1.1.2 CFA - Techniques](#cfatechniques), attestation has rarely targeted whole programs.
...
#### How it works
Similar to BLAST, CFA+ reserves 2 registers, `x27` and `x28`, naming them 
`RR` and `SR` respectively. They are used to encode/decode the return value in 
`x30` or the link register to prevent it from tampering.
...
3 types of code elements are then instrumented [@cfaplus]:

- *Function Prologues:*
All functions that are candidates for indirect calls start
with a `bti c`, followed by a block to verify the legitimacy of the caller. 
...
a mask value to all possible callers. If the mask value is present in `x15` at 
the time of call, it is legitimate and jump to the direct call label, 
else jump to an invalid address in `LR` using `blr x30`. 
(`LR` would have an address to a `bti j` so a call would cause a violation.)
After the direct call label, the `LR` is encoded and stored in the stack by using
`LR = LR` $\oplus$ `SR`.
- *Function Epilogues:*
`LR` is loaded and unmasked by XORing again with `SR` before returns.
- *Calls:* 
Direct and indirect calls are surrounded by instructions that encode and decode the `SR`.
The call instruction is preceeded by instructions to encode `SR` with a unique ID by XORing it.
The call is then followed by a `bti j` landing pad, and instructions to decode the `SR`.
...
`SR`'s value which would cause the `LR` to be corrupted in function epilogue.

It all looks like this:
```asm
foo:                    ; non-leaf callee
  bti c                 ; landing pad for indirect calls; foo prologue starts
  ldr x27, =mask        ; load the mask into RR
  and x27, x27, x15     ; bitwise AND between mask and ID (in x15)
  cmp x27, x15          ; compare result in x27 with ID
  b.eq foo_direct       ; if equal, resume foo by jumping to foo_direct
  blr x30               ; else jump to invalid address in LR
foo_direct:
  eor x30, x30, x28     ; mask LR (x30) using SR (x28)
  str x30, sp[-8]!      ; push LR to stack
  ...                   ; rest of foo prologue
  ...
  ...                   ; foo epilogue starts
  ldr x30, [sp], 8      ; load LR from stack
  eor x30, x30, x28     ; unmask LR using SR
  ret

bar:                    ; non-root caller
  ...                   ; bar prologue (similar to foo)
  ...
  ldr x15, =IDx         ; load ID of indirect call
  eor x28, x28, x15     ; encode SR using ID (in x15)
  blr x9                ; indirect call, may go to foo or baz
  bti j                 ; landing pad for ret
  ldr x15, =IDx         ; load ID again
  eor x28, x28, x15     ; decode SR using ID (in x15)
  ...
  ldr x15, =IDy         ; load ID of direct call
  eor x28, x28, x15     ; encode SR using ID (in x15)
  bl foo_direct         ; direct call to foo
  bti j                 ; landing pad for ret
  ldr x15, =IDy         ; load ID again
  eor x28, x28, x15     ; decode SR using ID (in x15)
  ...                   ; bar epilogue (similar to foo)
```

Calling shared libraries is also handled similar to this using `RR` instead of `SR`.
...
#### Limitations{#cfaplus-limitations}
...
Attackers would be able to figure out the IDs and required values for `SR` and `RR` using
reverse engineering and/or symbolic execution. If `SR`'s value for a given path
is known, it becomes easy to calculate the needed modifier value to change the
`LR` in stack so that when it gets "unmasked" with `SR`, it points to a desired address.
This enables the return to be controlled by the attacker. As returns do not need to
target `BTI` instructions [@arm-pacbti], attackers can jump to arbitrary points after that.
...
hijack a `br` or `blr` call, loading the target function's mask in `x15` would not
...
should be followed by `bti jc` to not cause a hardware exception. 
...
If it is there, or if the label acts as a `bti jc`, jumps to these labels can be
...
various side channels [@azad2020ios; @ravichandran2022pacman; @cai2023demystifying].
In contrast, the current design of CFA+ is transparent, i.e.,
secret-independent, resulting in a smaller attack surface." [@cfaplus - Section 7].

Logging for attestation is either done on violation, or on demand from the Vrf. And in both cases,
the value of `SR`, `RR` and `PC` are recorded. The Vrf performs a satisfiability check,
followed by `RR` consistency check for the paths, and a `SR` + `PC` consistency check.
